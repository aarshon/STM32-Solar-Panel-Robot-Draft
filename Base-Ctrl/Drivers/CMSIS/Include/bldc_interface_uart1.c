...
// Private functions
static void process_packet1(unsigned char *data, unsigned int len);
static void send_packet_bldc_interface1(unsigned char *data, unsigned int len);
 ...
 */
void bldc_interface_uart_init1(void(*func)(unsigned char *data, unsigned int len)) {
	// Initialize packet handler
	packet_init1(func, process_packet1, PACKET_HANDLER);

	// Initialize the bldc interface and provide a send function
	bldc_interface_init1(send_packet_bldc_interface1);
}
 ...
 */
void bldc_interface_uart_process_byte1(unsigned char b) {
	packet_process_byte1(b, PACKET_HANDLER);
}
 ...
 */
void bldc_interface_uart_run_timer1(void) {
	packet_timerfunc1();
}
...
static void process_packet1(unsigned char *data, unsigned int len) {
	...
	bldc_interface_process_packet1(data, len);
}
...
static void send_packet_bldc_interface1(unsigned char *data, unsigned int len) {
	...
	packet_send_packet1(data, len, PACKET_HANDLER);
}
