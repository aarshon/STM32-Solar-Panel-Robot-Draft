 ...
 * Patched:
 *   - Fixed include typo (upstream had `#include "StepMotor_lib.h".h"`).
 *   - Header retargeted to stm32f7xx.h for F767ZI.
 */
...
void startMotor(m_cnfg_s *motor_conf, direction_e direction, int step)
{
    if (motor_conf->m_set.state == MOTOR_OFF) {
        ...
        if (step != 0) {
            HAL_TIM_PWM_Start_IT(motor_conf->tim_handle, motor_conf->tim_channel);
        }
    }
}
