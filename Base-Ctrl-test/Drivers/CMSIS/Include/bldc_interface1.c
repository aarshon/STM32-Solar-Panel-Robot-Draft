...
// Private functions
void send_packet_no_fwd1(unsigned char *data, unsigned int len);
...
void bldc_interface_terminal_cmd1(char* cmd) {
	...
	send_packet_no_fwd1(send_buffer, len + 1);
}
...
void bldc_interface_set_duty_cycle1(float dutyCycle) {
	...
	send_packet_no_fwd1(send_buffer, send_index);
}
...
void bldc_interface_set_mcconf1(const mc_configuration *mcconf) {
	...
	send_packet_no_fwd1(send_buffer, ind);
}
...
void send_packet_no_fwd1(unsigned char *data, unsigned int len) {
	if (!forward_func) {
		bldc_interface_send_packet1(data, len);
	}
}
